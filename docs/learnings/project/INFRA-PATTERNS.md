...
**Rule**: when a Dependabot PR bumps a package that has sibling packages in the same family (`vitest` ↔ `@vitest/coverage-v8`, an ESLint core bump ↔ its plugin packages, etc.), check whether any other package in `package.json` peer-depends on the bumped package's *old* version before trusting a failing build to CI/deploy logs alone — `npm install` reproduces the real `ERESOLVE` error locally in seconds. Bump the sibling to match, verify `npm ls <pkg1> <pkg2>` resolves both to the same major, then push the fix onto the Dependabot branch — but expect Dependabot to possibly auto-close the PR right after a manual push (see debugging-discipline's note on this; `gh pr reopen` recovers it).
