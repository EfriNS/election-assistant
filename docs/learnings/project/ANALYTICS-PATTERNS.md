...
- **A text-cell row add can corrupt the MCP read path**: after adding a row with a text cell (`<h2>Context — volume &amp; completion trend</h2>`), `Get-Dashboard` on that board started failing permanently ("Unexpected error"), while `List-Dashboards` worked and the board remained duplicable (layout intact server-side) — and the corruption travels into duplicates. No MCP-side fix exists once the board is unreadable (row/cell deletes need ids you can no longer read). **Recovery path is the Mixpanel web UI** (different API): render the board, delete the stray text row manually. Rule: when a board becomes unreadable, STOP mutating it via MCP immediately.
