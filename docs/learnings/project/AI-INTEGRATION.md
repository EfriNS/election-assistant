...
**Correction, 2026-08-19: "genuine API errors (quota, network) are NOT retried" above was too broad — it silently included a real transient-retry case.** A 503 `UNAVAILABLE` ("This model is currently experiencing high demand ... Please try again later") is Google's own explicit transient-retry signal, categorically different from 429 quota exhaustion (which won't resolve on retry — the same request will fail again immediately). Caught via production Slack alerts: four `/api/follow-up` 503s in 2 days (2026-08-17/18), every one logging `retried=false`. Root cause: the retry-once loop above only wraps the *parse/shape* check — `ai.models.generateContent()`/`chat.sendMessage()` itself throws before that code runs on any thrown SDK error, bypassing the loop entirely regardless of whether the underlying error was actually transient. Fixed by wrapping the call site itself in a try/catch inside the same attempt loop and retrying once when `isTransientGeminiError(msg)` (new `lib/gemini-errors.ts`, matches `"503"`/`"UNAVAILABLE"`/`"overloaded"`) — 429/quota errors still propagate immediately on attempt 1, unretried, exactly as before. Applied identically to all three routes (`follow-up`, `score-topics`, `results`) since all three shared the exact same structural gap; `score-topics`/`results` were the more consequential of the two, since neither degrades gracefully the way `follow-up` does — an unretried 503 there hard-fails the user (500/`SERVER_ERROR`) instead of just skipping one follow-up question. Lesson: a retry-once loop's own doc comment asserting "X is not retried" needs re-checking whenever a new error shape shows up in production — "network errors" as a blanket category conflated a permanent failure (quota) with a self-described temporary one (503) that should have been split from the start.
