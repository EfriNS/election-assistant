...
21. **Don't string-compare raw `grep`/`cut` output against literals — quoting produces false mismatches. More generally: verify an extraction mechanism actually extracted something before trusting a comparison built on it** - Extracting a value from a config file with `grep "^VAR=" file | cut -d= -f2-` preserves any literal quote characters around the value (`VAR="x"` → extracted value is `"x"`, not `x`). Comparing that against an unquoted literal reports a false mismatch. To compare config values correctly: either `source`/`.` the file (shells strip quotes correctly during assignment) and compare the resulting variable, or use a real parser. This applies to any script comparing `.env`-style files, not just secrets.
    **Reinforced 2026-07-04, opposite-direction failure**: asked "do Preview and Production share a Mixpanel token?", ran `vercel env pull` for both environments and diffed — both came back `NEXT_PUBLIC_MIXPANEL_TOKEN=""`, concluded "same token" and reported it to the user as a finding. Wrong: `vercel env pull` in that sandbox was silently returning *every* encrypted secret as an empty string (confirmed by checking several other keys — all empty, while non-secret build vars like `TURBO_CACHE` came through fine), so the comparison was `"" === ""` — trivially true, zero evidence value. A same/equal result across an entire dataset, especially of secrets or anything normally non-empty, is itself a signal to check whether the extraction step worked at all (spot-check length/non-emptiness of a couple of known-real values) before treating the comparison's output as a fact. Caught only because the user pushed back with independent knowledge ("AFAIK there are different values") — retracted the claim rather than defending it. (#first:2026-07-02 #reinforced:2026-07-04)
...
22. **A project's own onboarding docs (CLAUDE.md, slash-command definitions, learning files) can themselves be stale — verify tooling instructions against real repo state before executing them literally** - `CLAUDE.md`, `.claude/commands/*.md`, and `docs/learnings/project/` all instructed running `pytest`/`black`/`ruff`/`mypy` inside a Python venv, deploying via `npx supabase functions deploy`, and running Playwright/i18n checks — none of which existed in the actual repo (a Next.js/TypeScript/Vitest app with no Python, no Supabase, no Playwright dependency, no `.github/workflows`). These were inherited from at least two unrelated project templates and never adapted. Didn't block work — real commands were substituted live by cross-checking `package.json` and the actual directory tree rather than following the doc — but the mismatch went unnoticed for many sessions because "read CLAUDE.md first" is followed as instruction, not verified as fact. Item #4 above says verify *external* identifiers before embedding them; this extends the same discipline inward — a repo's own foundational docs are a claim about the codebase's state, not the state itself, and are worth a quick sanity check (does `package.json` actually have this script? does this directory exist?) the first time they're invoked, not just trusted because they're positioned as ground truth. (#first:2026-07-03)
    ...
    **Reinforced 2026-07-06, higher-stakes instance**: `.env.example` documented a quota-check cron secret as `QUOTA_CRON_SECRET`, but the actual code (`process.env.CRON_SECRET`) and the platform's own convention (Vercel Cron Jobs auto-send `Authorization: Bearer $CRON_SECRET` only when the env var has that exact name) both used a different name. This is the same class of bug as the original finding — a doc claims something about the code that isn't true — but the consequence here isn't just a confused future session, it's a security control silently disabled for anyone who follows the setup instructions literally (the endpoint's `if (cronSecret)` check simply never fires). Setup/config docs (`.env.example`, README env-var tables) deserve the same "does this variable name actually appear in the code" grep-check as command-and-tooling docs, especially when the variable gates auth.

23. **A bug report describing behavior that contradicts what the current code should produce is a signal to verify *which build* is being observed before theorizing about the code** - A user's screenshot showed Hebrew text in an order that flatly contradicted the current source. Two rounds were spent generating plausible-sounding Unicode-bidi theories (a hyphen-before-a-quote as a "weak character" hazard, matching a real documented pattern in the same codebase) and "fixing" the text based on that reasoning — both fixes were wrong. The actual cause: the screenshot was from a deployment-specific preview URL permanently frozen on an old commit (a git-push-triggered platform like Vercel mints a new, unique, non-updating URL per deployment; only a separate "branch alias" URL tracks the latest push). Confirmed only by running the exact current commit locally and diffing the literal rendered output against the report — one command settled it after two wrong guesses. Lesson: a plausible root-cause theory that matches a documented pattern in the codebase is not evidence it's *this* bug — when a report seems to contradict the code you're looking at, check what's actually deployed/rendered (commit hash, timestamp, direct output inspection) before spending cycles on theories about why the code would produce that behavior. (#first:2026-07-05)
...
**Last Updated**: 2026-07-06 (reinforced #22 with a higher-stakes instance — a setup-doc/code env-var name mismatch that silently disabled an auth check, not just a stale-tooling-instruction confusion)
**Sessions Covered**: 22 retrospectives (2025-10-07 to 2026-07-06)
**Principles Count**: 19
