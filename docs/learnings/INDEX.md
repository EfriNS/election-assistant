...
- Debugging: A bug report that contradicts what the current code should produce means verify *which build/deployment* is being observed before theorizing about the code — two wrong bidi-theory fixes preceded finding the real cause (a stale, deployment-pinned preview URL) (see `universal/DEBUGGING.md` item 23, 2026-07-05)
...
- Architecture: Not every defensive-looking addition is a workaround — a retry on a *confirmed* external, non-deterministic failure (verified via direct reproduction, not assumed) is resilience, distinguishable from a workaround by whether the fix re-requests a clean result or parses/patches the broken one (see `universal/ARCHITECTURE.md` item 18, 2026-07-05)
...
- **Total Universal Principles**: 127 across all topics (CODING-PRINCIPLES: 12, TESTING: 35, PROCESS: 33, DEBUGGING: 19, ARCHITECTURE: 19, CI-CD: 9, AI-PROMPTS: 6) — inherited from the template, largely unaudited for this project
...
**Last Updated**: 2026-07-06 (pre-open-source cleanup session — repo structure pass (kebab-case fix, removed a stale generated doc, package.json metadata) followed by the three items TODO.md required before flipping the repo public: a comment/docs safety audit (clean overall; redacted a few internal identifiers with no reason to be public), a README accuracy pass, and a security re-assessment that found and fixed 4 real issues — most notably `/api/export-pdf` trusting raw client JSON with a `number`-typed field bypassing the file's own HTML-escape helper, rendered by a real headless-Chromium PDF export. Added universal ARCHITECTURE.md item 19 (a type annotation on parsed request JSON is a claim, not a runtime guarantee — check what an escape helper does *not* wrap) and universal CI-CD.md item 9 (flipping repo visibility exposes every branch/tag, not just the default branch; stale-but-merged branches are a hidden re-exposure surface if a history rewrite is ever attempted); reinforced DEBUGGING.md item 22 with a higher-stakes instance (a `.env.example`/code env-var name mismatch that silently disabled `/api/quota-check`'s auth check, found while doing the README pass — not just a stale-tooling-instruction confusion this time); added project/VAA-DESIGN.md items 80-81. Also scrubbed `docs/learnings/universal/*.md` itself of leftover narrative content from an unrelated prior project ("Contendre") — Docker/ChromaDB/Pydantic specifics, a named external founder, named companies — keeping every underlying principle intact. Separately: walked through (without executing) what a git-history rewrite vs. a private-historical-repo-plus-fresh-public-repo split would each actually involve, given the audit found lower-severity content in old commits; user decided to flip directly and keep full history intact.)

**Previously**: 2026-07-05 (critical-topic priority gate session — real user feedback root-caused via Langfuse+Mixpanel session cross-referencing resolved backlog #13 (cap קריטי to 2, follow-up depth scales with priority, a real score-cap gate instead of just a curve); diagnosed and hardened a production `/api/follow-up` JSON-parse failure (retry-once, explicit gershayim prompt instruction, full error logging, token-budget bump), confirmed via direct Gemini API reproduction to be a rare upstream glitch rather than a config issue — corrected project/AI-INTEGRATION.md's prior "guarantees valid JSON" claim about `responseJsonSchema`; added universal DEBUGGING.md item 23 (verify which build/deployment is being observed before theorizing about the code — two wrong bidi-theory fixes preceded finding a stale, deployment-pinned preview URL) and universal ARCHITECTURE.md item 18 (a retry on a *confirmed* external non-deterministic failure is resilience, not a workaround); added Verification Discipline and No Workarounds manifestations. Later the same day: same tester found follow-up option sets could be one-sided — a right-leaning security answer made every left-leaning party's grounded territorial-policy position invisible, because "close parties" (by current running score) gated both dimension-selection and grounding visibility. Added project/VAA-DESIGN.md item 79 — a "narrow to likely candidates" filter must bias towards relevance, not gate what's possible to surface, or it self-reinforces the user's first answer; extracted `selectSuggestedDimension()` and confirmed the fix via live API reproduction, not just logical inspection)

**Previously**: 2026-07-04 (pre-launch legal/privacy risk review session — added project/VAA-DESIGN.md items 77-78: for an Israeli VAA, undisclosed data practices are the real legal exposure (Amendment 13 names political opinions as specially-sensitive data; the mako "מצפן הבחירות" precedent was about disclosure, not perceived bias), and gate a feature on a not-yet-true external dependency behind one flag rather than shipping a temporarily-dead link; reinforced item 72 with a second live-fetch-plus-Wayback-check verification (shas.org.il: search results said "the party site," live fetch said `ECONNREFUSED`, Wayback said abandoned since 2022); added universal PROCESS.md item 42 — don't default to team-scale SLA commitments in user-facing copy without checking who's actually the operator (removed a "5 business days" response commitment after the user, a solo maintainer, pointed out it didn't fit); added a Debugging manifestation to Cross-Cutting Theme 1 (Verification Discipline))
