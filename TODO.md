...
- **Follow-up + results JSON parse errors fixed — structured-output schema** — Two production `SERVER_ERROR` Slack alerts traced (via Langfuse) to Gemini emitting unescaped Hebrew gershayim/acronym characters (צה"ל, מו"מ) inside JSON string values, breaking `JSON.parse`. Fixed both `follow-up` and `results` routes (the latter was more exposed — no `responseMimeType` at all — and generates Hebrew prose instructed to quote party platforms verbatim, the same risk) by adding `responseJsonSchema` (constrained decoding guarantees valid JSON regardless of content) + regression tests for both. `score-topics` audited too: its output is pure numbers/null with no Hebrew string values, so it's not exposed to this specific bug — real but lower-priority cleanup remains there (see backlog #2). (2026-07-02)
...
2. **`score-topics` JSON parsing cleanup (low urgency, not the same bug)** — `follow-up` and `results` both fixed with `responseJsonSchema` (2026-07-02). `app/api/score-topics/route.ts` audited and found *not* exposed to the same Hebrew-gershayim bug — its output is pure `{"topicId.partyId": number | null}`, no free-text Hebrew string values. It does show a different symptom of unconstrained JSON generation: `parseScores` already works around the model sometimes emitting `+2` instead of `2` (invalid JSON number syntax) via a regex, and extracts JSON via `raw.match(/\{[\s\S]*\}/)` rather than a direct parse. A `responseJsonSchema` with `type: "number"` score fields would structurally prevent the `+2` quirk and let both workarounds be deleted — worthwhile cleanup, not an active bug.
