...
- **Fix: `lib/parties.ts` drifted from grounding data (party website/platform links), low-contrast text** — Hadash and Otzmah Yehudit showed "ללא מצע רשמי" incorrectly despite having real official-current grounding; fixed both. Shas's `website` pointed to an unrelated senior-housing site — the "correct" domain (`shas.org.il`) turned out dead too (live `ECONNREFUSED`, Wayback last snapshot Nov 2022) — set to empty. Darkened unreadable `text-gray-300` → `text-gray-500`/`400` for several "unknown"/date labels across the results page and PDF export. (2026-07-04)
